...
const IndividualHoot = () => {
  ...
  const getHootById = async () => {
    await axios.get(`${BaseURL}/hoot/public/${id}`).then((response) => {
      ...
      {hoot && (
        <div className="individualHoot">
          {hoot.map((hoot) => {
            ...
            if (hoot.fontFamilyStyle.includes("fontFamily")) {
              fontFamilyStyle = JSON.parse(hoot.fontFamilyStyle);
              ...
              var fontFamily = hoot.fontFamilyStyle || "Arial";
