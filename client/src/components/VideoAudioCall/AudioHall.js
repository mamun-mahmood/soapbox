...
class AudioHall extends Component {
    constructor(props) {
        super(props);
    ...
    componentDidMount = () => {
        ...
        var profilePic=this.props.location.state.profilePic;
        var participants = {};
        ...
        const dynamicVideoLayout = () => {
            ...
            myarray.forEach((userid) => {
                if (myarray.length == 1) {
                    ...
                    // document.getElementById(userid).poster=profilePic
        ...
        const receiveVideo = (userid, username) => {
            ...
            video.onclick = () => {
                ...
                // document.getElementById("maxVideoStream").poster=document.getElementById(userid).poster
        ...
        const sendEmail = async () => {
            await axios.post(`${BaseURL}/nodemailer/inviteHandler`, {
                body: JSON.stringify({
                    ...
                    link: `https://megahoot.net/${uuidv4()}/ReceptionAudio/${this.props.location.state.hallId}/${uuidv4()}`,
                }),
            }).then((response) => {
    ...
    waiters: this.state.waiters.filter(
      (user) => user !== waiter
                    ...
                    <div style={{ flex: 3, position: 'relative', left: '-50px' }} >
                        <video id="maxVideoStream" onMouseDown={(event)=>event.preventDefault ? event.preventDefault() : event.returnValue = false} style={{ zIndex: 1, position: 'absolute' }} width="100%" controlls  ></video>
                        <img onContextMenu={(e)=>e.preventDefault()} onMouseDown={(event)=>event.preventDefault ? event.preventDefault() : event.returnValue = false} style={{ width: '100%', height: "100%", zIndex: 2, position: 'absolute' }} src={frame} />
...
export default withRouter(AudioHall)
